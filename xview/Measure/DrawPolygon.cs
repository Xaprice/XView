...
namespace DrawTools
{
    ...
    class DrawPolygon : DrawTools.DrawLine
    {
        ...
        public override void Draw(Graphics g, double zoomFactor = 1.0)
        {
            ...
            {
                x1 = (int)(Math.Round(((Point)enumerator.Current).X * zoomFactor));
                y1 = (int)(Math.Round(((Point)enumerator.Current).Y * zoomFactor));
            }

            while (enumerator.MoveNext())
            {
                x2 = (int)(Math.Round(((Point)enumerator.Current).X * zoomFactor));
                y2 = (int)(Math.Round(((Point)enumerator.Current).Y * zoomFactor));
